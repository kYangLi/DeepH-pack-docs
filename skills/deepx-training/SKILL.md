...
在超算环境中，严禁在登录节点直接运行高负载任务。必须通过作业调度系统（如 Slurm, PBS）提交。提交任务前，最好向用户索要提交脚本模板，以快速得知作业提交队列、可用环境等信息。
...
### 4.2 最佳实践
*   **预测试 (Smoke Test)**：在进行大规模提交之前，建议先运行 1-2 个小任务进行调试，以确保脚本逻辑的正确性。对于超大规模模型训练（例如，单个 Epoch 的运行时间超过 30 分钟），此步骤可以选择跳过。如果需要在集群上进行此测试，**严禁在登录节点直接进行测试**，请务必通过作业提交系统提交测试作业。在执行该测试任务时，请务必将命令的超时阈值 (Timeout) 调整为 15 分钟，以确保任务能够顺利完成。

...
*   **⚠️ 路径提示**：配置文件支持相对路径（相对于 TOML 文件所在目录），但为避免解析歧义，**强烈推荐使用绝对路径**。
...
**注意事项：**
1.  **作业命名隔离**：为每一批次作业添加唯一的名称前缀（如随机动物名），以实现同账户下不同实验组的命名空间隔离，防止混淆。
2.  **获取提交模板**：启动作业前，请务必要求用户/管理员提供标准的作业提交模板文件，以确保准确掌握作业队列、环境加载等关键配置信息。

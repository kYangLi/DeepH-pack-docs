...
Parameters of the training process are configured through a TOML-formatted file. Each key systematically governs specific aspects of the computational workflow. 
...
*   **Description**: This flag enables or disables the validation check for DFT training data. When set to `true`, the system performs a validation check to ensure that the training data is correctly formatted and contains the necessary information. This check is crucial for ensuring the integrity and quality of the training data, but will much slow down the graph building process.
*   **Default**: `false`
*   **Type**: `<BOOL>`
...
*   **Description**: 本参数在训练大模型时非常关键，因为大模型是一个epoch一存，有时可能好几个星期都不会存一次模型，这是非产危险的。这个参数设置了，每隔多少步保存一次模型，比如设置为1000，那么每隔1000步就会保存一次模型，这样即使训练大模型，也不会导致模型丢失。如果设为-1，则不会保存中间模型。
*   **Default**: `-1`
*   **Type**: `<INT>`
---
...
*   **Description**: The neural network architecture.
    ...
    *   `eagle` is an advanced architecture (typically $\sim$5M parameters) with both node and edge features, which is suitable for tasks of DFT Hamiltonian learning that requires high accuracy.
    *   `eagleplus` (also named `accurate`) is an advanced architecture (typically $\sim$10M parameters) with both node and edge features, which is suitable for tasks of DFT Hamiltonian learning that requires high accuracy.
    *   `albatross` is an advanced architecture (typically $\sim$20M parameters) with both node and edge features, which is suitable for tasks of DFT Hamiltonian learning that requires high accuracy.
    *   `owl` is an advanced architecture (typically $\sim$5M parameters) with node features, which is suitable for tasks of DFT Force Field learning that requires high accuracy.
    *   `petrel` is an advanced architecture (typically $\sim$20M parameters) with node features, which is suitable for tasks of DFT Force Field learning that requires high accuracy.
    *   `swift` is an auto-encoder.
*   **Default**: `"normal"`
*   **Type**: [`"sparrow"`, `"fast"`, `"eagle"`, `"accurate"`, `"eagleplus"`, `"albatross"`, `"owl"`, `"swift"`, `"petrel"`]
...
*   **Description**: Irreducible representations of the neural network features, which ensure the equivariance of the network. \\
    For `sparrow`, the `Irreps` can be set to ``$\cdots$\verb|x0e+|$\cdots$\verb|x1e+|$\cdots$\verb|x2e+|$\cdots$\verb|x3e+|$\cdots$\verb|x4e+|$\cdots$''. The channel $l$ has parity $(-1)^{l}$. \\
...
*   **Description**: Irreducible representations of the latent neural network features, which ensure the equivariance of the network. Useful for albatross net.\\
*   **Default**: `<Invalid-Input>`
*   **Type**: `<STRING>`
...
*   **Description**: The cutoff distance for the latent neural network.
*   **Default**: `<Invalid-Input>`
*   **Type**: `<STRING>`
...
*   **Description**: 启动BS3B层，使得模型对元素区分度更高。
*   **Default**: `2`
*   **Type**: `<INT>`
...
*   **Description**: 启动BS3B层表示，其设置格式与`data.graph.common_orbital_types`一致，一般需要设置大于common_orbital_types。
*   **Default**: `2`
*   **Type**: `<INT>`

...
**特别注意事项！**
1. **严禁全量终止作业（Scope Safety）**：执行作业清理时，必须且只能针对特定的 Job ID 进行操作。**绝对禁止执行“清除当前用户所有作业”这类指令**，以确保该用户账号下其他正在运行的非测试业务不受影响。
...
3.  **获取提交模板**：启动作业前，**务必要求用户/管理员提供标准的作业提交模板文件**，以确保准确掌握作业队列、环境加载等关键配置信息。
4.  **资源一致性**：调度系统申请的 GPU 数量必须与 TOML 配置文件中 system.device 的参数严格保持一致。

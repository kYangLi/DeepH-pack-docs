...
在超算环境中，**严禁在登录节点直接运行任何高负载任务**。必须通过作业调度系统（如 Slurm, PBS）提交。提交任务前，建议向用户索要提交脚本模板，以快速得知作业提交队列、环境加载等信息，例如：
```bash
#!/bin/bash
#
...
source /data/home/deeph/.uvenv/deeph-1.2.1-cu129/bin/activate

deeph-train train-____.toml
```

### 3.4 超算环境作业安全与操作规范！
1. **严禁全量终止作业（Scope Safety）**：执行作业清理时，必须只针对特定的 Job ID 进行操作。**绝对禁止执行“清除当前用户所有作业(scancel -u [username])” 这类指令**，以确保该用户账号下其他正在运行的非测试业务不受影响。
2. **作业命名隔离与冲突检测**：务必为每一批次作业应用**唯一的随机名称前缀**（例如随机动物名 panda_），以建立独立的命名空间。**关键步骤**： 在应用前缀前，**必须检索当前正在运行的作业列表**，确保新生成的前缀未被占用（无重复）。严禁使用已存在的 ID 前缀，以彻底防止多实验组之间的上下文混淆。
3.  **精准批量终止（Precise Termination）**：执行作业清理时，必须采用基于独特前缀的过滤机制。建议组合使用 grep（筛选前缀）、awk（提取 Job ID）和 xargs（批量传递）构建命令行管道，从而精确锁定并终止目标作业，误删其他无关作业。
4.  **获取提交模板**：启动作业前，**务必要求用户/管理员提供标准的作业提交模板文件**，以确保准确掌握作业队列、环境加载等关键配置信息。
5.  **资源一致性**：调度系统申请的 GPU 数量必须与 TOML 配置文件中 system.device 的参数严格保持一致。
...
### 5.3 优化器与训练策略
*   **`init_learning_rate`**： 优化器最关键的超参数，直接决定模型收敛的速度与稳定性。学习率设置过大可能导致数值不稳定（如 Loss 出现 NaN）或梯度爆炸；设置过小则会导致收敛极其缓慢，甚至陷入局部最优解无法收敛。实际取值需根据模型参数量进行微调，经验推荐值如下：
    *   分子材料：通常设为 1e-3 至 1e-2
    *   晶体材料：通常设为 1e-4 至 1e-3
